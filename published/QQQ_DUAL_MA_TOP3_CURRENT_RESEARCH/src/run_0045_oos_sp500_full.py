...
OUT=ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0045";NY=ZoneInfo("America/New_York");KEY=["symbol","target_ts","role"]
def build():
 ...
 daily=pd.read_parquet(OUT/"sp500_daily_all_adjusted_apr_augtd.parquet");daily.date=pd.to_datetime(daily.date);ext_dates=pd.DatetimeIndex(sorted(daily.loc[daily.date.between("2026-05-01","2026-08-10"),"date"].unique()));eval_dates=ext_dates
 ...
 rows=[];prevw=discovery_executed[-1].copy()
...
def replay():
 ...
 d=pd.read_parquet(OUT/"gross_daily.parquet");d.date=pd.to_datetime(d.date);d["cost"]=d.date.map(t.groupby("session_date").cost.sum()).fillna(0);d["net_pnl"]=d.gross_pnl-d.cost;eq=1+d.net_pnl.cumsum();peak=np.maximum.accumulate(np.r_[1.,eq.to_numpy()])[1:];monthly=d.set_index("date").net_pnl.resample("ME").sum();report={"status":"completed","net_fixed_base_pct":float(100*d.net_pnl.sum()),"monthly_net_fixed_base_pct":{str(k.date()):float(100*v) for k,v in monthly.items()},"maximum_drawdown_pct":float(-100*(eq/peak-1).min()),"positive_days":int((d.net_pnl>0).sum()),"negative_days":int((d.net_pnl<0).sum()),"best_day_pct":float(100*d.net_pnl.max()),"worst_day_pct":float(100*d.net_pnl.min()),"quote_coverage":1.0,"turnover":float(t.delta_weight.sum()),"maximum_loaded_date":"2026-08-10","rows_after_authorized_end":0};t.to_parquet(OUT/"quote_fill_ledger.parquet",index=False);d.to_parquet(OUT/"oos_daily_net.parquet",index=False);(OUT/"oos_report.json").write_text(json.dumps(report,indent=2)+"\n");print(json.dumps(report,indent=2))
