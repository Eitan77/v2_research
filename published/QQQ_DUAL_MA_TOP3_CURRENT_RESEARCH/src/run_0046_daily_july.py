...
OUT=ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0046"
...
def build():
 ...
 daily=pd.read_parquet(ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0042"/"daily_all_adjusted_apr_augtd.parquet");daily.date=pd.to_datetime(daily.date)
 ext_dates=pd.DatetimeIndex(sorted(daily.loc[daily.date.between("2026-05-01","2026-07-31"),"date"].unique()))
 eval_dates=ext_dates[ext_dates>=pd.Timestamp("2026-07-01")]
 ...
 carry=np.load(ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0030"/"weights_daily_rebalance.npy")[-2].copy();eval_pos={d:i for i,d in enumerate(eval_dates)}
