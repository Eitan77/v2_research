...
OUT=ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0041"
...
def build():
 ...
 daily=pd.read_parquet(OUT/"daily_all_adjusted_apr_jul.parquet");daily.date=pd.to_datetime(daily.date)
 ext_dates=pd.DatetimeIndex(sorted(daily.loc[daily.date.between("2026-05-01","2026-07-31"),"date"].unique()))
 eval_dates=ext_dates[ext_dates>=pd.Timestamp("2026-06-01")]
 if eval_dates.max()>pd.Timestamp("2026-07-31"):raise RuntimeError("authorized range breached")
 ...
 mem=con.execute("select try_cast(date as date) date,symbol,arg_max(is_member,try_cast(ingested_at as timestamp)) is_member from qqq_pit_membership_daily where try_cast(date as date) between date '2026-05-01' and date '2026-07-31' group by 1,2").fetchdf()
 raw=con.execute("select date,symbol,arg_max(close,try_cast(ingested_at as timestamp)) as raw_close,arg_max(volume,try_cast(ingested_at as timestamp)) as raw_volume from bars_1d where date between date '2026-05-01' and date '2026-07-31' and feed='sip' and adjustment='raw' group by 1,2").fetchdf();con.close()
 ...
 periods=ext_dates.to_period("W-FRI");signal_idx=[i for i in range(n) if i==n-1 or periods[i+1]!=periods[i]]
 ...
 carry=np.load(ROOT/"campaigns"/"CAM-0611"/"artifacts"/"RUN-0040"/"executed_weights.npy")[-1].copy();eval_pos={d:i for i,d in enumerate(eval_dates)}
 ...
 build_report={"status":"passed","discovery_maximum_date":str(pd.Timestamp(p.dates.max()).date()),"evaluation_minimum_date":str(eval_dates.min().date()),"evaluation_maximum_date":str(eval_dates.max().date()),"rows_after_july":0,"membership_maximum_date":str(membership_max.date()),"membership_carry_forward_sessions":int((eval_dates>membership_max).sum()),"targets":targets,"quote_roles":len(ledger)};(OUT/"build_report.json").write_text(json.dumps(build_report,indent=2)+"\n");print(json.dumps(build_report,indent=2))
...
def benchmark():
 con=duckdb.connect(r"D:\AlgoResearch\data\catalog.duckdb",read_only=True)
 q=con.execute("select date,arg_max(open,try_cast(ingested_at as timestamp)) as px_open,arg_max(close,try_cast(ingested_at as timestamp)) as px_close from bars_1d where symbol='QQQ' and feed='sip' and adjustment='raw' and date between date '2026-05-01' and date '2026-07-31' group by date order by date").fetchdf();con.close();q.date=pd.to_datetime(q.date)
 monthly={str(k):float(100*(g.px_close.iloc[-1]/g.px_open.iloc[0]-1)) for k,g in q.groupby(q.date.dt.to_period("M"))};report={"method":"first_session_open_to_last_session_close","monthly_return_pct":monthly,"may_july_return_pct":float(100*(q.px_close.iloc[-1]/q.px_open.iloc[0]-1)),"minimum_date":str(q.date.min().date()),"maximum_date":str(q.date.max().date()),"rows_after_july":0};(OUT/"qqq_benchmark.json").write_text(json.dumps(report,indent=2)+"\n");print(json.dumps(report,indent=2))
