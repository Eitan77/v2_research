...
from run_0034_staircase_ratchet import OUT,ratchet,SPECS
NY=ZoneInfo("America/New_York");IDS=("control","stair15_5_1close","stair15_5_2close","stair10_5_2close")
def weights():
 p,score,mask,sig,base,sma50,ranks=base_context();return p,{n:ratchet(p,sig,base,SPECS[n])[0] for n in IDS}
def ledgers():
...
def cache(label):
 frames=[]
 for run in ("RUN-0030","RUN-0031","RUN-0032","RUN-0033"):
...
def replay():
 ...
 x=f.symbol.eq("ALXN")&f.session_date.eq(pd.Timestamp("2021-07-21"))&f.side.eq("sell")
 if x.any():
  base=ROOT/"campaigns"/"CAM-0600"/"artifacts"/"RUN-0044";refs=pd.read_parquet(base/"terminal_reference_quotes.parquet");terms=pd.read_parquet(base/"terminal_exception_quotes.parquet");a=refs[refs.symbol.eq("ALXN")].iloc[0];b=terms[terms.symbol.eq("ALXN")].iloc[0];f.loc[x,"reference_mid"]=(float(a.bid_price)+float(a.ask_price))/2;f.loc[x,"bid_price"]=float(b.bid_price);f.loc[x,"ask_price"]=float(b.ask_price);f.loc[x,"session_date"]=pd.Timestamp("2021-07-20")
