...
def build_parallel_block(
    ...
    temp_root = output_path.parent / f".{output_path.stem}_parts"
    ...
    with ProcessPoolExecutor(max_workers=config.feature_workers) as pool:
        for part, symbols in enumerate(groups):
            part_path = temp_root / f"part_{part:02d}.parquet"
            futures[pool.submit(_build_symbol_part, canonical_path, part_path, config, specs, symbols)] = part
        for complete, future in enumerate(as_completed(futures), 1):
            future.result()
            progress_path.write_text(json.dumps({
                "stage": "feature_cache_parallel",
                "feature_block": output_path.name,
    ...
    built = [spec for spec in specs if spec.name in columns]
    shutil.rmtree(temp_root)
    return built


def _build_symbol_part(
    canonical_path: Path,
    part_path: Path,
    config: ScanConfig,
    specs: list[FeatureSpec],
    ...
    frame, _ = build_features(bars, config, specs, symbol_local=True)
