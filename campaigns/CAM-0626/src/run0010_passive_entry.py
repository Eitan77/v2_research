...
def main():
 src=pd.read_parquet(A/'RUN-0006'/'entry_audit.parquet');src.date=pd.to_datetime(src.date);src=src[['date','symbol','entry']].copy();jobs=[{'session_date':d.strftime('%Y-%m-%d'),'symbol':s,'window':'entry','kind':k} for d,s in src[['date','symbol']].itertuples(index=False,name=None) for k in ('quotes','trades')];manifest=[]
 with ThreadPoolExecutor(max_workers=16) as pool:
  fs={pool.submit(eng.fetch_window,j):j for j in jobs}
  for i,f in enumerate(as_completed(fs),1):
   try:manifest.append(f.result())
   except Exception as e:manifest.append({**fs[f],'error':str(e)})
 ...
 for x in src.itertuples():
  ds=x.date.strftime('%Y-%m-%d');order=eng.base.session_timestamp(ds,5);target=1/float(x.entry) if np.isfinite(x.entry) and x.entry>0 else 0;z=eng.simulate_passive_fast(lookup.get((ds,x.symbol,'quotes'),[]),lookup.get((ds,x.symbol,'trades'),[]),'sell',order,target,300);fills.append({'date':x.date,'symbol':x.symbol,'target_qty':target,**z})
