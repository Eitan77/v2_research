...
def run_campaign(campaign_id, panels, fundamental, preflight):
    output = CAMPAIGNS / campaign_id / "artifacts" / "RUN-0002"
    variants = build_adaptations(campaign_id, panels, fundamental)
    records = []
    for variant in variants:
        for cost in COSTS_BPS:
            metrics, daily, monthly, yearly, symbols = evaluate_weights(
                variant.panel, variant.weights, cost,
            ...
            save_variant(
                output, f"{variant.variant_id}__cost_{cost:g}bps", record,
                daily, monthly, yearly, symbols, save_detail=float(cost) == 2.0,
            )
    ...
    report = {
        ...
        "broker_margin": False,
        "preflight": preflight,
        "frozen_contract": str(CAMPAIGNS / "CAM-0600" / "ADAPTATION_CONTRACT.yaml"),
        "interpretation_blockers": [],
    }
    write_json(output / "execution_report.json", report)
    print(f"{campaign_id}: {len(variants)} adaptations, best 2bps={best['variant_id']} net={best['net_simple_return']}", flush=True)
