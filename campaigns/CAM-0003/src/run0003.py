...
SYMBOLS = ["SPY", "QQQ", "IWM", "DIA", "XLF", "XLK", "SMH", "TLT", "TQQQ", "SOXL"]
MORNINGS = [15, 30, 60]
HOLDS = [15, 30, 60]
...
def extract(temp: Path) -> pd.DataFrame:
    ...
    WITH ranked AS (
      ...
      SELECT symbol,date,try_cast(timestamp AS TIMESTAMPTZ) ts,open,close,volume
...
def summarize(d: pd.DataFrame, symbol: str, component: str, morning: int, hold: int, state: str) -> dict:
    ...
    signal_return = (
        morning_close/x["previous_close"]-1 if component == "combined_previous_close"
    ...
    cost = 10.0 if symbol in {"TQQQ", "SOXL"} else 5.0
    ...
    row = {
        "symbol": symbol, "asset_class": "leveraged" if cost == 10 else "unlevered",
        "component": component, "morning": morning, "hold": hold, "state": state,
        "cost": cost, "signal_days": int(active.sum()), "net": float(pnl.sum()),
...
def main() -> None:
    ...
    required=["previous_close","open_0930","close_m15","close_m30","close_m60",
              "entry_h15","entry_h30","entry_h60","session_close"]
    nulls={c:int(eval_[c].isna().sum()) for c in required}
    if any(nulls.values()): raise RuntimeError(f"price attrition {nulls}")
    eval_.to_parquet(a.output_dir/"etf_daily_features.parquet", index=False)
    rows=[
        summarize(eval_,symbol,component,morning,hold,state)
        for symbol in SYMBOLS for component in COMPONENTS for morning in MORNINGS
        for hold in HOLDS for state in STATES
    ]
    if len(rows)!=540: raise RuntimeError("variant mismatch")
    grid=pd.DataFrame(rows).sort_values("15m_avg_month",ascending=False)
    grid.to_csv(a.output_dir/"etf_state_grid.csv",index=False)
    diagnostics={
        "rows":len(eval_),"symbols":SYMBOLS,"price_nulls":nulls,
        "max_date":str(eval_["date"].max().date()),
        "positive_variants":int((grid["net"]>0).sum()),
        "positive_unlevered_variants":int(((grid["net"]>0)&(grid["asset_class"]=="unlevered")).sum()),
        "leaders":grid.head(30).to_dict(orient="records"),
    }
    (a.output_dir/"diagnostics.json").write_text(json.dumps(diagnostics,indent=2)+"\n")
    contract={
        "executed_variant_count":540,"expected_variant_count":540,
        "symbols":SYMBOLS,"components":COMPONENTS,"mornings":MORNINGS,
        "holds":HOLDS,"states":STATES,"unlevered_cost":5,"leveraged_cost":10,
