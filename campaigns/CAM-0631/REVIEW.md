...
## RUN-0007 execution-repair conclusion

The prespecified repair also failed. The first six confirmation dates were the only development data. All 1,296 combinations of frozen 5/10/20% toxicity cuts, one/two/three-cent spread floors, one/ten/100-share queue sizes, 250/500 ms latency, one/five/15-second entry lives, base/conservative queue models, five/15-second marketable exits, and one/two-basis-point exit slippage completed. No row combined at least 20 fills with positive fixed-base P&L in both the first-three and last-three development-date blocks. The untouched final six dates were therefore not evaluated.

The few positive totals were not credible neighborhoods. The raw maximum earned 2.24 bp per fill but had only nine fills and its later development block was negative. The best row satisfying the 20-fill floor had 37 fills, lost 1.74 bp per fill, and was negative in both chronological blocks. Raising the absolute spread floor did not repair the trade-off: the best two-cent row remained negative, while the best three-cent total came from only two fills.

Conclusion: the source-inspired toxicity signal is real enough to preserve as a research lead, but the historical passive execution claim is rejected under the available queue-evidence and cost model. The campaign does not claim paper or live profitability. Full-session broker-specific shadow calibration could test acknowledgement latency and actual queue behavior in a future campaign, but it cannot rescue this historical result and is outside the evidence available here.
